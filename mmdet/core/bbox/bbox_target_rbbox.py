...
from .transforms_rbbox import mask2poly, get_best_begin_point, polygonToRotRectangle_batch \
    , best_match_dbbox2delta, hbb2obb_v2
...
def bbox_target_rbbox_single(pos_bboxes,
    ...
    # change to 2.0 format
    # labels = pos_bboxes.new_zeros(num_samples, dtype=torch.long)

    labels = pos_bboxes.new_full((num_samples,), num_classes, dtype=torch.long)
    ...
    if num_pos > 0:
        ...
        if with_module:
            ...
            #                                target_stds)
            pos_bbox_targets = coder.encode(pos_ext_bboxes, pos_gt_obbs, 'dbbox2delta')
        else:
            # pos_bbox_targets = dbbox2delta_v3(pos_ext_bboxes, pos_gt_obbs, target_means,
            #                                   target_stds)
            pos_bbox_targets = coder.encode(pos_ext_bboxes, pos_gt_obbs, 'dbbox2delta_v3')
...
def rbbox_target_rbbox_single(pos_rbboxes,
    ...
    labels = pos_rbboxes.new_full((num_samples,), num_classes, dtype=torch.long)
    ...
    if num_pos > 0:
        ...
        # import pdb
        # pdb.set_trace()
        pos_bbox_targets = best_match_dbbox2delta(pos_rbboxes, pos_gt_rbboxes, coder)
