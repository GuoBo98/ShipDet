...
def best_match_dbbox2delta(Rrois, gt, coder):
    """
    :param Rrois: (x_ctr, y_ctr, w, h, angle)
            shape (n, 5)
    ...
    # Here, use a choose beste match angle to Rrois, minmum dthea, similar to choose best point instead
    ...
    bbox_targets = coder.encode(Rrois, gt_boxes_new, 'dbbox2delta_v2')
    return bbox_targets
...
def dbbox_flip(dbboxes, img_shape):
    ...
    # flip angle
    bbox_num = flipped.size()[0]
    cls_box_num = flipped.size()[1]
    flipped = flipped.view(-1, 5)
    pos_inds = flipped[:, 4] > np.pi
    neg_inds = flipped[:, 4] < 0
    flipped[pos_inds, 4] -= np.pi
    flipped[neg_inds, 4] += np.pi
    flipped = flipped.view(bbox_num, cls_box_num)

    flipped[:, 4::5] = np.pi - flipped[:, 4::5]
...
def dbbox_mapping_back(dbboxes, img_shape, scale_factor, flip):
    ...
    :return:
    """
    scale_factor_tensor = torch.tensor(scale_factor, device=dbboxes.device)
    new_dbboxes = dbbox_flip(dbboxes, img_shape) if flip else dbboxes
    if new_dbboxes.size(1) > 5:
        import pdb
        pdb.set_trace()
        print('new_dbboxes.size(1)', new_dbboxes.size(1))
    new_dbboxes = new_dbboxes.view(new_dbboxes.size(0), -1, 5)

    # new_dbboxes[..., 0::5] = new_dbboxes[..., 0::5] / scale_factor_tensor
    # new_dbboxes[..., 1::5] = new_dbboxes[..., 1::5] / scale_factor_tensor
    # new_dbboxes[..., 2::5] = new_dbboxes[..., 2::5] / scale_factor_tensor
    # new_dbboxes[..., 3::5] = new_dbboxes[..., 3::5] / scale_factor_tensor
    new_dbboxes[:, :, :4] /= scale_factor_tensor
    new_dbboxes = new_dbboxes.view(dbboxes.shape)
    return new_dbboxes
...
def mask2poly_single(binary_mask):
    ...
    :param binary_mask: binary_mask of one obj
    :return: poly:polygon of one obj
    """
    try:
        contours, hierarchy = cv2.findContours(binary_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        # contour_lens = np.array(list(map(len, contours)))
...
def mask2poly(binary_mask_list):
    ...
    :param binary_mask_list: list of binary mask of one img
    :return: list(polys): polys of objs in one img　The polygon stored in format [x1, y1, x2, y2, x3, y3, x4, y4]
            shape [num_boxes, 8]
    """
...
def gt_mask_bp_obbs(gt_masks, with_module=True):
    """
    :param gt_masks: The polygon stored in format [x1, y1, x2, y2, x3, y3, x4, y4]
            shape [num_boxes, 8] one img
...
def gt_mask_bp_obbs_list(gt_masks_list):
    ...
    :param gt_masks_list: list of masks of all imgs
    :return:Rotated Rectangle list in format [cx, cy, w, h, theta]
            shape [[num_rot_recs, 5], [num_rot_recs, 5], [num_rot_recs, 5], [num_rot_recs, 5]]
    """
...
def get_best_begin_point(coordinate_list):
    ...
    :param coordinate_list: polygon list [num_box 8]
    :return: best_coordinate_list: np.array polygon after mapping to bbox
    """
